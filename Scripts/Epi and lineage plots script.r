...
######### Epi curve
#Read in case, death, Re and genomic data
data1<-read_excel('owid-covid-data.xlsx')
...
pEpi_Moz<-ggplot()+
  ...
  geom_line(data=data1, aes(x=days, y=total_deaths, color='Deaths'), size=1.4)+
...
#create lineage plot
P_pangolin3<-ggplot(data=subset(gisaid_Moz, !is.na(pango)), aes(x = date,fill=pango, color = pango))+
